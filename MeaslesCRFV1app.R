...
## Measles Case Report Form (CDC CRFV1) Only
...
# {r Filepath, message=FALSE, warning=FALSE, include=FALSE}
# Define file path
excel_path <- "data/outbreak_data.xlsx"
if (!file.exists(excel_path)) {
    ...
    df_init <- data.frame(
        ...
        age = numeric(),
        onset_date = as.Date(character()),  # store as date in Excel
        vacc_status = character(),
        stringsAsFactors = FALSE
    )
    write.xlsx(df_init, excel_path, rowNames = FALSE)
}

# Reactive flag for reloading data
data_updated <- reactiveVal(Sys.time())




# {r UI Script, message=FALSE, warning=FALSE, include=FALSE}
ui <- fluidPage(
    titlePanel("Measles Case Report Form (CDC CRF V1)"),
    mainPanel(
        
        # 1) The form to collect basic info
        textInput("patient_name", "Patient Name"),
        numericInput("age", "Age", value = 0, min = 0),
        dateInput("onset_date_text", "Date of Rash Onset (MM/DD/YYYY)", value = format(Sys.Date(), "%m/%d/%Y")),
        ...
        wellPanel(
            ...
            selectInput("sex_at_birth", "Sex at Birth", c("Male", "Female", "No answer")),
        ...
        wellPanel(
            ...
            radioButtons("symptom_conjunctivitis", "Swollen/red/itchy eyes (conjunctivitis)?", c("Yes", "No", "Unknown")),
            ...
            radioButtons("symptom_low_platelets", "Low platelets (Thrombocytopenia)?", c("Yes", "No", "Unknown")),
            radioButtons("symptom_encephalitis", "Brain inflammation (Encephalitis)?", c("Yes", "No", "Unknown")),
            textAreaInput("symptom_other", "Other symptoms", height = "40px")
        ),
        
        wellPanel(
            h3("7. Healthcare Interaction"),
            radioButtons("healthcare_visited", "Visited healthcare provider?", c("Yes", "No", "Unknown")),
        ...
        wellPanel(
            ...
            radioButtons("pregnancy_status", "(If female) Are you pregnant?", c("Yes", "No", "Unknown")),
            radioButtons("immune_weakened", "Weakened immune system?", c("Yes", "No", "Unknown")),
            textAreaInput("immune_conditions", "Conditions or medications leading to weakened immunity", height = "40px")
        ),
        
        wellPanel(
            h3("9. Epidemiologic Investigation"),
            radioButtons("recent_travel", "Travel outside U.S. past 21 days?", c("Yes", "No", "Unknown")),
            ...
            radioButtons("known_contact", "Contact with known measles case/symptoms before rash?", c("Yes", "No", "Unknown")),
        ...
        wellPanel(
            ...
            radioButtons("vaccine_received", "Received measles-containing vaccine?", c("Yes", "No", "Unknown")),
            ...
            radioButtons("pep_within_timeframe", "PEP given within 3/6 days of exposure?", c("Yes", "No", "Unknown")),
            selectInput("ig_admin_method", "If IG given, route", c("Intramuscular", "Intravenous", "Unknown"))
        ),
        
        wellPanel(
            h3("14. Laboratory Information"),
            radioButtons("testing_performed", "Measles testing performed?", c("Yes", "No", "Unknown")),
        ...
        hr(),
        # >>> Download Button <<<
        downloadButton("download_data", "Download Current Excel File")
    )
)
...
# {r Server Script, message=FALSE, warning=FALSE, include=FALSE}
server <- function(input, output, session) {
    
    # Minimal loader if needed
    load_data <- reactive({
        data_updated()
        if (!file.exists(excel_path)) return(data.frame())
        read.xlsx(excel_path, colNames=TRUE)
    })
    
    # Save a new case
    observeEvent(input$save, {
        # 1) read existing data
        df <- read.xlsx(excel_path, colNames=TRUE)
        # ensure columns are character/numeric as needed
        df$case_id      <- as.character(df$case_id)
        df$patient_name <- as.character(df$patient_name)
        df$age          <- suppressWarnings(as.numeric(df$age))
        df$onset_date   <- as.character(df$onset_date)
        ...
        if (is.na(onset_date_clean) || onset_date_clean > Sys.Date()) {
            showNotification("Invalid or future date. Please use MM/DD/YYYY format.", type="error")
            return(NULL)
        }
        ...
        new_row <- data.frame(
            ...
            age          = input$age,
            onset_date   = format(onset_date_clean, "%Y-%m-%d"),
        ...
        new_row <- new_row %>%
            mutate(
                ...
                patient_name        = input$patient_name_form, # overwrites above
                report_date_hd      = as.character(input$report_date_hd),
                invest_start_date   = as.character(input$invest_start_date),
                db_entry_date       = as.character(input$db_entry_date),
                ...
                fever_onset_date    = as.character(input$fever_onset_date),
                ...
                healthcare_visit_date   = as.character(input$healthcare_visit_date),
                healthcare_facility     = input$healthcare_facility,
                hospitalized            = input$hospitalized,
                hospital_name           = input$hospital_name,
                hospital_admit_date     = as.character(input$hospital_admit_date),
                hospital_discharge_date = as.character(input$hospital_discharge_date),
                patient_death           = input$patient_death,
                death_date              = as.character(input$death_date),
                ...
                travel_depart_date   = as.character(input$travel_depart_date),
                countries_visited    = input$countries_visited,
                travel_return_date   = as.character(input$travel_return_date),
                ...
                vaccine_dose1_date  = as.character(input$vaccine_dose1_date),
                vaccine_dose2_date  = as.character(input$vaccine_dose2_date),
                vaccine_dose3_date  = as.character(input$vaccine_dose3_date),
                pep_received        = input$pep_received,
                pep_type            = input$pep_type,
                pep_date            = as.character(input$pep_date),
    ...
    output$download_data <- downloadHandler(
        ...
        content = function(file) {
            # Simply copy the existing outbreak_data.xlsx to 'file'
            # 'file' is a temp path that Shiny uses to send the downloaded file
